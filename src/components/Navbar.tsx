    ...
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${scrolled ? 'py-3' : 'py-5'}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className={`glass rounded-2xl px-4 h-14 flex items-center justify-between ${scrolled ? 'shadow-2xl' : ''}`}>
          <Link href="/" className="group flex items-center gap-2.5 interactive-bounce">
          ...
          {user && (
            <div className="hidden sm:flex items-center gap-2">
              <Link href="/assessments" className="interactive-bounce px-4 py-2 rounded-xl text-sm font-semibold text-slate-300 hover:text-white hover:bg-white/[.06] transition flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <>
                <Link href={`/portfolio/${user.id}`} className="group interactive-bounce flex items-center gap-2.5 rounded-xl px-2.5 py-1.5 border border-white/[.07] bg-white/[.025] hover:bg-white/[.07] transition">
                ...
                <button onClick={handleLogout} aria-label="Log out" className="w-10 h-10 interactive-bounce grid place-items-center rounded-xl text-slate-500 hover:text-pink-300 hover:bg-pink-400/10 transition">
