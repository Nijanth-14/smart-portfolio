  ...
  return (
    <div className="relative min-h-screen overflow-hidden flex items-center justify-center p-5 sm:p-8 pt-28">
      ...
      <main className="relative z-10 w-full max-w-2xl">
        <div className="gradient-border glass rounded-[2rem] p-7 sm:p-12 text-center reveal-up">
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border text-xs font-black tracking-[.12em] ${isVerified ? 'bg-lime-300/10 border-lime-300/20 text-lime-300' : 'bg-pink-300/10 border-pink-300/20 text-pink-300'}`}>
            <span className={`w-2 h-2 rounded-full ${isVerified ? 'bg-lime-300 pulse-dot' : 'bg-pink-300'}`} />
          ...
          <div className="mt-10">
            ...
            <h1 className="mt-5 text-3xl sm:text-4xl font-black text-electric">{profile?.full_name}</h1>
            <Link href={`/portfolio/${credential.user_id}`} className="mt-2 inline-flex items-center gap-1 text-cyan-300 hover:text-white transition">
          ...
          <div className="mt-5 gradient-border glass rounded-2xl p-7">
